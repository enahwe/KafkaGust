...
* By passing a set of arguments (e.g, number of gusts, number of messages in each gust, size of each message, the message's template, compression codec, batch mode, ...) it becomes easy and quick to launch a test campaign on to any Kafka environment by calling a single script responsible for sending gusts of messages and for producing statistics files
