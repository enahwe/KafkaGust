...
Finally by defining a set of features (e.g, number of messages, template of message, size of each message, message variables, messages keys, batch mode, ...) it becomes easy to test quickly any Kafka environment by creating a simple single script (copy-past and edit a shell) containing in itself a campaign of tests.
